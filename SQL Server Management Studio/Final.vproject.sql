...
CREATE TABLE Prescription_ProfG_FP (
    ...
    Notes VARCHAR(255),
    Create_dt DATETIME NOT NULL DEFAULT GETUTCDATE(),
    FOREIGN KEY (PatientID) REFERENCES Patient_ProfG_FP(PatientID),
    FOREIGN KEY (DoctorID) REFERENCES Doctor_ProfG_FP(DoctorID),
    FOREIGN KEY (MedicationID) REFERENCES Medication_ProfG_FP(MedicationID)
);
GO
...
CREATE TABLE Pharmacist_ProfG_FP (
...
INSERT INTO.Pharmacist_ProfG_FP 
...
CREATE TABLE Transaction_ProfG_FP (
    ...
    FOREIGN KEY (PrescriptionID) REFERENCES Prescription_ProfG_FP(PrescriptionID),
    FOREIGN KEY (PharmacistID) REFERENCES Pharmacist_ProfG_FP(PharmacistID)
);
GO

INSERT INTO Transaction_ProfG_FP 
...
FROM Transaction_ProfG_FP;
GO


CREATE TABLE Payment_ProfG_FP (
...
FROM Payment_ProfG_FP;
GO


CREATE TABLE Appointment_ProfG_FP (
...
INSERT INTO Appointment_ProfG_FP 
...
CREATE OR ALTER VIEW vw_PatientMasked_ProfG_FP
AS
SELECT 
    PatientID,
    FirstName,
...
SELECT * FROM vw_PatientMasked_ProfG_FP;
GO


CREATE OR ALTER PROCEDURE sp_AddPrescription_ProfG_FP
    @PatientID INT,
    @DoctorID INT,
    @MedicationID INT,
    @Dosage VARCHAR(50),
...
EXEC sp_AddPrescription_ProfG_FP
    @PatientID = 1,
    @DoctorID = 1,
    @MedicationID = 1,
    @Dosage = '250mg',
...
CREATE OR ALTER TRIGGER trg_PrescriptionAudit_ProfG_FP
...
BEGIN
    ...
    BEGIN
        -- Insert action
        INSERT INTO AuditLog_ProfG_FP (ActionType, TableName, RecordID)
...
WHERE PrescriptionID = (
    SELECT TOP 1 PrescriptionID FROM Prescription_ProfG_FP ORDER BY PrescriptionID DESC
);
GO

DELETE FROM Prescription_ProfG_FP
WHERE PrescriptionID = (
    SELECT TOP 1 PrescriptionID FROM Prescription_ProfG_FP ORDER BY PrescriptionID DESC
);
GO
...
SELECT * FROM AuditLog_ProfG_FP;
GO


CREATE OR ALTER FUNCTION fn_CalculatePatientAge_ProfG_FP
(
    @DOB DATE
)
RETURNS INT
...
JOIN Patient_ProfG_FP Pt ON P.PatientID = Pt.PatientID
JOIN Doctor_ProfG_FP D ON P.DoctorID = D.DoctorID
...
IF OBJECT_ID('Inventory_ProfG_FP', 'U') IS NOT NULL
    DROP TABLE Inventory_ProfG_FP;
GO
